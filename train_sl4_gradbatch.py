...
args = dict(
    select_every=dict(count=5, mode="epoch"),
    warm_start = 20,
    # warm_every = 21,
    warm_end = 0,
...
dataname = "cifar10_normalised"

data_dir = f"/home/ganesh/AAI_prayas/AAAI/data/{dataname}"
...
if(dataname == "mnist"):
    ...
    trainset, testset, _, _ = gen_dataset("../data","tinyimagenet","dss")
elif dataname == "cifar":
    # Define data transforms
    transform_train = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        ...
        for epoch in range(start_epoch, num_epochs+1):
            ...
            for batch_idx, (features, targets, images) in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}")):
                ...
                if(self.do_warm_end(args, epoch, allow_zero=False)):
                    ...
                    image_grads, val_grads = submod.calc_grads_gradbatchv2(model, criterion, bak_trainloader, valloader)
                    num_queries = val_grads.shape[0]
                    print("n shape",n, image_grads.shape, val_grads.shape, num_queries)
                    image_grads = image_grads[:n]
                    if(args["init_func"] is not None):
                        diversity = [
                            DisparityMinFunction(n = n, data = image_grads, mode="sparse", metric="euclidean",num_neighbors=5),
                            DisparitySumFunction(n = n, data = image_grads, mode="sparse", metric="euclidean",num_neighbors=5),
                            LogDeterminantFunction(n, mode="sparse", lambdaVal = 0.1,  data = image_grads, metric="euclidean", num_neighbors=5),
                        ]
                        representation = [
                            LogDeterminantFunction(n, mode="sparse", lambdaVal = 0.1,  data = image_grads, metric="euclidean", num_neighbors=5),
                            FacilityLocationFunction(n, mode="sparse", data = image_grads, metric="euclidean", num_neighbors=5),
                        ]
                        funcs = self.get_funcs(diversity + representation, len(diversity), args, epoch)
                    else: 
                        funcs = [
                            submodlib.GraphCutMutualInformationFunction(n = n, data = image_grads, queryData=val_grads, metric="euclidean", num_queries=num_queries),
                            submodlib.LogDeterminantMutualInformationFunction(n, lambdaVal = 0.1,  data = image_grads,queryData=val_grads, metric="euclidean",num_queries=num_queries),
                            submodlib.FacilityLocationMutualInformationFunction(n, data = image_grads,queryData=val_grads, metric="euclidean",num_queries=num_queries),
                            submodlib.ConcaveOverModularFunction(n, data = image_grads,queryData=val_grads, metric="euclidean",num_queries=num_queries),
                            GraphCutFunction(n = n, data = image_grads, mode="sparse", metric="cosine",num_neighbors=10, lambdaVal = 0.1),
                            LogDeterminantFunction(n, mode="sparse", lambdaVal = 0.1,  data = image_grads, metric="cosine", num_neighbors=10),
                            FacilityLocationFunction(n, mode="sparse", data = image_grads, metric="cosine", num_neighbors=10),
                        ]
                    # funcs = [
                    ...
                    #                                             val_images=val_images, val_labels=val_labels, val_grads=val_grads)
                    mode, greedyFinal, _best_arm = submod.eps_greedy_composition_batched(model, val_samples, 
                                                                criterion, global_step, funcs, submod_budget, moment_sum, args, 
                                                                bak_trainloader, greedyOnly=False, 
                                                                opt_grads=opt_grads, val_sim="mean",
                                                                val_images=None, val_labels=None, val_grads=val_grads)
                    best_arm = _best_arm
