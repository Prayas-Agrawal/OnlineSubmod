...
args = dict(
    select_every=dict(count=2, mode="epoch", use_prev_best_arm_for_next_batch= False),
    warm_start = 0,
...
class TrainClassifier:
    ...
    def do_dss_epoch(self, args, epoch):
    ...
    def train(self, **kwargs):
        ...
        for epoch in range(start_epoch, self.cfg.train_args.num_epochs+1):
            ...
            idxs = []
            gammas = []
            budget_batches = int(args["submod_budget"]*len(trainloader))
            budget_per_batch = budget_batches//args["batch_size"]
            opt_subset = {
                "images": None,
                "features": None,
                "labels": None,
            ...
            if(self.do_dss_epoch(args, epoch)):
                print("****Doing Subset Selection on full data****")
                dataloader = trainloader

            for batch_idx, (features, targets, images) in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{self.cfg.train_args.num_epochs}")):
                ...
                if(args["warm_start"] <= epoch+1 and self.do_dss(args, step, epoch)):
                    # print("do 1")
                    ...
                    idxs.extend([n*batch_idx + i for i in submod_indices])
                    gammas.extend([n*batch_idx + i for i in submod_weights])
                ...
                #     final_labels = subset["labels"][submod_indices]
                # else:
                #     self.opt_subset[batch_idx] = {"images": images, "features": features, "labels": targets}
                ...
                if(args["warm_start"] <= epoch+1 and self.do_dss(args, step, epoch) and batch_idx == len(dataloader)-1):
                    print("****Refreshing****")
                    print("Lens", len(idxs))
