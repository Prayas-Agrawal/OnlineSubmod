...
class DataSelectionStrategy_onlinesubmod(object):
    ...
    def get_greedy_lists(self, budget, clone_params, orig_params,clone_model, orig_model, trn_grads, val_grads, batch_scores, batch_size):
        ...
        funcs = [
            DisparityMinFunction(n = n, sijs=batch_scores, mode="dense", metric="euclidean"),
            DisparitySumFunction(n = n, sijs=batch_scores, mode="dense", metric="euclidean"),
            LogDeterminantFunction(n, mode="dense", lambdaVal = 0.1,  sijs=batch_scores, metric="euclidean"),
            FacilityLocationFunction(n, mode="dense", sijs=batch_scores, metric="euclidean", separate_rep=False),
        ]
        ...
        lamb = 0.1
        pi = 1.1
        greedyFinal, best_arm = self.best_submod_bandit(greedyList, eta_n, moment_sum, subset_grads, val_grads, 0)
        #within batch
        submod_indices = [greedyFinal[i][0] for i in range(len(greedyFinal))]
        # self.prev_opt_subset[batch_idx] = [i for i in submod_indices]
        # global_indices = [batch_idx*(batch_size) + i for i in submod_indices]
