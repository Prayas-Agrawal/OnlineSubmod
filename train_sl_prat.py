...
def extract_features(dataloader, load_from=None):
    ...
    if(load_from is not None and os.path.exists(load_from)):
        ...
        #     if features[i].dtype == torch.float:
        #         features[i] = tf.convert_image_dtype(features[i],torch.uint8)
        #         labels[i] = tf.convert_image_dtype(labels[i],torch.uint8)
        #         images[i] = tf.convert_image_dtype(images[i],torch.uint8)
...
class TrainClassifier:
    ...
    def train(self, **kwargs):
        ...
        trainloader = DataLoader(train_features_dataset, batch_size=batch_size, shuffle=False, pin_memory=False)

        valloader = DataLoader(test_features_dataset, batch_size=batch_size, shuffle=False, pin_memory=False)

        testloader = DataLoader(test_features_dataset, batch_size=batch_size, shuffle=False, pin_memory=False)
	
        train_eval_loader = DataLoader(train_features_dataset, batch_size=batch_size, shuffle=False, pin_memory=False)

        val_eval_loader = DataLoader(test_features_dataset, batch_size=batch_size, shuffle=False, pin_memory=False)

        test_eval_loader = DataLoader(test_features_dataset, batch_size=batch_size, shuffle=False, pin_memory=False)
        ...
        def get_samples():
            val_images = None
            val_labels = None
            for _, labels, images in valloader:
                if images.dtype == torch.uint8:
                    images = images.float()
                images = images.to(device)
        ...
        for epoch in range(start_epoch, self.cfg.train_args.num_epochs+1):
            ...
            for features, targets, images in tqdm(dataloader, desc=f"Epoch {epoch+1}/{self.cfg.train_args.num_epochs}"):
                ...
                submod_budget = int(n*args["submod_budget"])
                final_images =  images
                final_labels = targets
                if(self.do_dss(args, step, epoch, args["warm_start"])):
                    # print("do 1")
                    # with torch.autocast(device_type=device):
                        # Eps greedy bandit selection
                    greedyFinal, _best_arm = submod.eps_greedy_composition(model, subset, len_opt, val_samples, 
                                                                criterion, step, funcs, submod_budget, moment_sum, args, optimizer="StochasticGreedy")
                    best_arm = _best_arm
